...
class AbstractExplorer(ABC):
    ...
    initialized based on the get_service_endpoint method that has to be implemented by child.\n

    This is designed to have an implementation of multiple Nasa APIs with a correct level of abstraction and\n
    also that the explorer has no idea of what is the type of data_access he has.
    """
    def __init__(self, api_key: str) -> None:
        super().__init__()
        self.data_access = NasaAPIAccess(api_key=api_key, api_service=self.get_service_endpoint())
